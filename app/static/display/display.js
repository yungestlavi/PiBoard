...
const RENDERERS = {
  ...
  weather(inner, cfg) {
    const draw = async () => {
      ...
      inner.innerHTML = "";
      if (!d.ok) { inner.innerHTML = `<div class="weather-meta">Meteo non disponibile</div>`; return; }
      const wrap = document.createElement("div"); wrap.className = "weather-wrap";
      wrap.innerHTML =
        `<div class="weather"><span class="weather-icon">${d.icon}</span>` +
        `<div><div class="weather-temp">${d.temp}${d.unit}</div>` +
        `<div class="weather-meta">${d.desc}${cfg.place ? " · " + escapeHtml(cfg.place) : ""}</div>` +
        `<div class="weather-sub">↑${d.hi}° ↓${d.lo}°${d.humidity != null ? " · 💧" + d.humidity + "%" : ""}</div>` +
        `</div></div>`;
      if ((d.forecast || []).length) {
        const fc = document.createElement("div"); fc.className = "weather-fc";
        d.forecast.forEach((f) => {
          const c = document.createElement("div"); c.className = "fc-day";
          c.innerHTML = `<div class="fc-d">${f.day}</div><div class="fc-i">${f.icon}</div>` +
            `<div class="fc-t">${f.hi}°<span>${f.lo}°</span></div>`;
          fc.append(c);
        });
        wrap.append(fc);
      }
